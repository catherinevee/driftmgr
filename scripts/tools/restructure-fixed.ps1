# DriftMgr File Structure Migration Script (PowerShell) - Fixed Version
...
if (-not $DryRun) {
    ...
    # Move scripts - Fixed version
    Write-Status "Moving scripts..."
    if (Test-Path "scripts") {
        # Move shell scripts
        Get-ChildItem "scripts" -Filter "*.sh" -ErrorAction SilentlyContinue | ForEach-Object {
            $filename = $_.Name
        ...
        # Move Python scripts
        Get-ChildItem "scripts" -Filter "*.py" -ErrorAction SilentlyContinue | ForEach-Object {
            Move-FileWithBackup $_.FullName "scripts/tools/$($_.Name)"
        }
        
        # Move batch files
        Get-ChildItem "scripts" -Filter "*.bat" -ErrorAction SilentlyContinue | ForEach-Object {
            Move-FileWithBackup $_.FullName "scripts/tools/$($_.Name)"
        }
    }

    # Move PowerShell scripts from root
    Write-Status "Moving PowerShell scripts from root..."
