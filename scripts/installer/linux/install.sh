...
update_credentials() {
    print_status "Auto-detecting DriftMgr credentials..."
    ...
    if [[ -f "$PROJECT_ROOT/bin/driftmgr" ]]; then
        print_status "Running credential auto-detection..."
        if "$PROJECT_ROOT/bin/driftmgr" credentials auto-detect; then
            print_success "Credentials auto-detection completed!"
        else
            print_error "Failed to auto-detect credentials"
            exit 1
        fi
    else
        print_error "DriftMgr binary not found at $PROJECT_ROOT/bin/driftmgr"
        print_error "Please install DriftMgr first."
        exit 1
    fi
}
