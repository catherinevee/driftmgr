...
update_credentials() {
    print_status "Updating DriftMgr credentials..."
    
    # Get the directory where this script is located
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    
    # Check if driftmgr binary exists
    if [[ -f "$SCRIPT_DIR/bin/driftmgr" ]]; then
        print_status "Running credential auto-detection..."
        "$SCRIPT_DIR/bin/driftmgr" credentials auto-detect
        print_success "Credentials auto-detection completed!"
    elif [[ -f "$SCRIPT_DIR/bin/driftmgr.exe" ]]; then
        print_status "Running credential auto-detection..."
        "$SCRIPT_DIR/bin/driftmgr.exe" credentials auto-detect
        print_success "Credentials auto-detection completed!"
    else
        print_error "DriftMgr binary not found. Please install DriftMgr first."
        exit 1
    fi
}
...
show_help() {
    ...
    echo "  -c, --update-credentials Update cloud credentials"
...
run_windows_installer() {
    ...
    if [[ "$UPDATE_CREDENTIALS" == "true" ]]; then
        PS_ARGS="$PS_ARGS -UpdateCredentials"
    fi
...
run_linux_installer() {
    ...
    if [[ "$UPDATE_CREDENTIALS" == "true" ]]; then
        BASH_ARGS="$BASH_ARGS --update-credentials"
    fi
