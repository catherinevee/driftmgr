...
func NewHandlers() *Handlers {
	...
	return &Handlers{
		...
		discoveryHub:         nil, // Should be set by the server
	}
}

// SetDiscoveryHub sets the discovery hub for the handlers
func (h *Handlers) SetDiscoveryHub(hub *discovery.DiscoveryHub) {
	h.discoveryHub = hub
}
...
func (h *Handlers) GetDiscoveryV1(c *gin.Context) {
	// Get real discovery results from the hub
	results := h.discoveryHub.GetAllResources()
	
	// Calculate totals
	totalResources := len(results)
	
	// Get unique providers from actual resources
...
// handleClearCache handles requests to clear the resource cache
func (s *Server) handleClearCache(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(map[string]interface{}{
		"success": true,
		"message": "Cache cleared successfully. Starting new discovery...",
	})

	// Trigger auto-discovery of all accounts in background
	go s.triggerFullDiscovery()
}

// triggerFullDiscovery starts discovery for all configured providers and regions
func (s *Server) triggerFullDiscovery() {
	// Detect configured credentials
	configuredProviders := s.credDetector.DetectAll()
	
	for _, cred := range configuredProviders {
		if cred.Status == "configured" {
			provider := strings.ToLower(cred.Provider)
			
			// Get all regions for provider
			var regions []string
			switch provider {
			case "aws":
				regions = []string{
					...
					// Opt-in regions excluded to avoid auth errors
				}
			case "azure":
				regions = []string{
			...
			s.discoveryHub.StartDiscovery(discoveryReq)
		}
	}
}
