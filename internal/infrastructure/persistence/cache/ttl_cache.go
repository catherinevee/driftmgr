...
// SimpleCacheEntry represents a cached item with expiration in the simple cache
...
type TTLCache struct {
	items      map[string]*SimpleCacheEntry
	mu         sync.RWMutex
...
func NewTTLCache(defaultTTL time.Duration, maxSize int) *TTLCache {
	cache := &TTLCache{
		items:      make(map[string]*SimpleCacheEntry),
...
func (c *TTLCache) Clear() {
	...
	c.items = make(map[string]*SimpleCacheEntry)
