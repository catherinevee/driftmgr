...
func NewDigitalOceanDiscoverer(region string) (*DigitalOceanDiscoverer, error) {
	...
	if token == "" {
		...
		if homeDir != "" {
			// Try multiple possible doctl config locations
			configPaths := []string{
				filepath.Join(homeDir, "AppData", "Roaming", "doctl", "config.yaml"),
				filepath.Join(homeDir, ".config", "doctl", "config.yaml"),
				filepath.Join(homeDir, ".doctl", "config.yaml"),
			}
	...
	if token == "" {
		return nil, fmt.Errorf("no DigitalOcean token found. Please set DIGITALOCEAN_ACCESS_TOKEN or configure doctl")
	}
...
func (d *DigitalOceanDiscoverer) Discover() ([]models.Resource, error) {
	// Client should never be nil if constructor succeeded
	if d.client == nil {
	...
	fmt.Printf("  [DigitalOcean] Starting resource discovery...\n")

	// Discover Droplets
	droplets, err := d.discoverDroplets(ctx)
	if err != nil {
		fmt.Printf("  [DigitalOcean] Warning: Failed to discover droplets: %v\n", err)
	} else if len(droplets) > 0 {
		fmt.Printf("  [DigitalOcean] Found %d droplets\n", len(droplets))
	...
	if err != nil {
		fmt.Printf("  [DigitalOcean] Warning: Failed to discover Kubernetes clusters: %v\n", err)
	} else if len(clusters) > 0 {
		fmt.Printf("  [DigitalOcean] Found %d Kubernetes clusters\n", len(clusters))
	...
	if err != nil {
		fmt.Printf("  [DigitalOcean] Warning: Failed to discover databases: %v\n", err)
	} else if len(databases) > 0 {
		fmt.Printf("  [DigitalOcean] Found %d databases\n", len(databases))
	...
	if err != nil {
		fmt.Printf("  [DigitalOcean] Warning: Failed to discover load balancers: %v\n", err)
	} else if len(loadBalancers) > 0 {
		fmt.Printf("  [DigitalOcean] Found %d load balancers\n", len(loadBalancers))
	...
	if err != nil {
		fmt.Printf("  [DigitalOcean] Warning: Failed to discover volumes: %v\n", err)
	} else if len(volumes) > 0 {
		fmt.Printf("  [DigitalOcean] Found %d volumes\n", len(volumes))
	...
	if err != nil {
		fmt.Printf("  [DigitalOcean] Warning: Failed to discover spaces: %v\n", err)
	} else if len(spaces) > 0 {
		fmt.Printf("  [DigitalOcean] Found %d spaces\n", len(spaces))
	...
	if err != nil {
		fmt.Printf("  [DigitalOcean] Warning: Failed to discover domains: %v\n", err)
	} else if len(domains) > 0 {
		fmt.Printf("  [DigitalOcean] Found %d domains\n", len(domains))
	...
	if err != nil {
		fmt.Printf("  [DigitalOcean] Warning: Failed to discover projects: %v\n", err)
	} else if len(projects) > 0 {
		fmt.Printf("  [DigitalOcean] Found %d projects\n", len(projects))
	...
	if err != nil {
		fmt.Printf("  [DigitalOcean] Warning: Failed to discover VPCs: %v\n", err)
	} else if len(vpcs) > 0 {
		fmt.Printf("  [DigitalOcean] Found %d VPCs\n", len(vpcs))
	...
	if err != nil {
		fmt.Printf("  [DigitalOcean] Warning: Failed to discover firewalls: %v\n", err)
	} else if len(firewalls) > 0 {
		fmt.Printf("  [DigitalOcean] Found %d firewalls\n", len(firewalls))
		resources = append(resources, firewalls...)
	}

	fmt.Printf("  [DigitalOcean] Total resources discovered: %d\n", len(resources))
	...
	for {
		...
		for _, droplet := range droplets {
			...
			if droplet.Networks != nil && len(droplet.Networks.V4) > 0 {
				resource.Attributes["ip_address"] = droplet.Networks.V4[0].IPAddress
			}
			
			// Add creation time if available
			if droplet.Created != "" {
				resource.Attributes["created_at"] = droplet.Created
			}
