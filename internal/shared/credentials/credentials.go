...
// IsConfiguredWithContext checks if a provider has configured credentials (with context)
func (m *Manager) IsConfiguredWithContext(ctx context.Context, provider string) (bool, error) {
	if creds, ok := m.providers[provider]; ok {
