...
func (awc *AzureWindowsCLI) DiscoverResourcesUsingCLI(ctx context.Context) ([]models.Resource, error) {
	...
	discoveryFuncs := []struct {
		...
		{"ManagedIdentities", awc.discoverManagedIdentitiesViaCLI},
		{"NetworkWatchers", awc.discoverNetworkWatchersViaCLI},
	}
...
func (awc *AzureWindowsCLI) discoverNetworkWatchersViaCLI(ctx context.Context) ([]models.Resource, error) {
	cmd := exec.CommandContext(ctx, "az", "network", "watcher", "list", "--output", "json")
	output, err := cmd.Output()
	if err != nil {
		return nil, fmt.Errorf("failed to discover network watchers: %w", err)
	}

	var watchers []map[string]interface{}
	if err := json.Unmarshal(output, &watchers); err != nil {
		return nil, fmt.Errorf("failed to parse network watchers: %w", err)
	}

	var resources []models.Resource
	for _, watcher := range watchers {
		resource := models.Resource{
			ID:       getStringFromCLI(watcher, "id"),
			Name:     getStringFromCLI(watcher, "name"),
			Type:     "azure_network_watcher",
			Provider: "azure",
			Region:   getStringFromCLI(watcher, "location"),
			Tags:     convertCLITags(watcher["tags"]),
		}
		resources = append(resources, resource)
	}
