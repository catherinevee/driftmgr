...
// DetectDrift is an alias for Detect to match expected API
func (d *Detector) DetectDrift(ctx context.Context, currentResources []models.Resource, options DetectionOptions) (*DetectionResult, error) {
	return d.Detect(ctx, currentResources, options)
}
...
// loadTerraformState loads resources from a Terraform state file
func (d *Detector) loadTerraformState(stateFile string) ([]models.Resource, error) {
	...
	// Convert Terraform resources to our Resource model
	var resources []models.Resource

	// Process root module resources
	if state.Values != nil && state.Values.RootModule != nil {
		for _, tfResource := range state.Values.RootModule.Resources {
			resource := d.convertTerraformResource(tfResource)
			resources = append(resources, resource)
		}

		// Process child module resources
		for _, childModule := range state.Values.RootModule.ChildModules {
			moduleResources := d.processChildModule(childModule)
...
// convertTerraformResource converts a Terraform resource to our Resource model
func (d *Detector) convertTerraformResource(tfResource *tfjson.StateResource) models.Resource {
	resource := models.Resource{
		ID:       tfResource.Address,
		Name:     tfResource.Name,
		Type:     tfResource.Type,
		Provider: tfResource.ProviderName,
	}

	// Extract attributes
	if tfResource.AttributeValues != nil {
		resource.Attributes = tfResource.AttributeValues
		...
		// Set state from various possible attribute names
		for _, stateKey := range []string{"state", "status", "lifecycle_state"} {
			if state, ok := tfResource.AttributeValues[stateKey].(string); ok {
	...
	resource.Metadata = map[string]string{
		"terraform_mode":    string(tfResource.Mode),
		"terraform_address": tfResource.Address,
	}

	if tfResource.SchemaVersion > 0 {
		resource.Metadata["schema_version"] = fmt.Sprintf("%d", tfResource.SchemaVersion)
	}

	return resource
}

// processChildModule processes resources from a child module
func (d *Detector) processChildModule(module *tfjson.StateModule) []models.Resource {
	var resources []models.Resource
