...
func generateMarkdownReport(report *drift.TerraformDriftReport, outputPath string, includeRecommendations, includeCosts bool) {
	...
	if report.Analysis != nil {
		...
		if len(recommendations) > 0 {
			md.WriteString("### Immediate Actions\n\n")
		...
		// Add drift patterns if available
		if report.Analysis.Patterns != nil && len(report.Analysis.Patterns) > 0 {
			md.WriteString("### Detected Patterns\n\n")
			for _, pattern := range report.Analysis.Patterns {
				md.WriteString(fmt.Sprintf("- %s\n", pattern))
			}
			md.WriteString("\n")
		}
		
		// Add preventive actions from predictions
		if report.Predictions != nil && len(report.Predictions.PreventiveActions) > 0 {
			md.WriteString("### Preventive Actions\n\n")
			for _, action := range report.Predictions.PreventiveActions {
				md.WriteString(fmt.Sprintf("- %s\n", action))
			}
			md.WriteString("\n")
		}
...
func generateRecommendations(report *drift.TerraformDriftReport) []Recommendation {
	var recommendations []Recommendation

	if report == nil || report.DriftItems == nil {
		return recommendations
	}

	// Analyze drift items and generate recommendations
	driftByType := make(map[string]int)
	driftBySeverity := make(map[string]int)
	hasSecurityDrift := false
	hasTagDrift := false
	hasScalingDrift := false
	hasNetworkDrift := false

	for _, item := range report.DriftItems {
		driftByType[item.ResourceType]++
		driftBySeverity[item.Severity]++

		// Check for specific drift types
		for _, diff := range item.Differences {
			if strings.Contains(strings.ToLower(diff.Path), "security") ||
				strings.Contains(strings.ToLower(diff.Path), "encryption") ||
				strings.Contains(strings.ToLower(diff.Path), "public") {
				hasSecurityDrift = true
			}
			if strings.Contains(strings.ToLower(diff.Path), "tag") {
				hasTagDrift = true
			}
			if strings.Contains(strings.ToLower(diff.Path), "instance") ||
				strings.Contains(strings.ToLower(diff.Path), "capacity") ||
				strings.Contains(strings.ToLower(diff.Path), "size") {
				hasScalingDrift = true
			}
			if strings.Contains(strings.ToLower(diff.Path), "subnet") ||
				strings.Contains(strings.ToLower(diff.Path), "vpc") ||
				strings.Contains(strings.ToLower(diff.Path), "network") {
				hasNetworkDrift = true
			}
		}
	}

	// Generate recommendations based on findings
	if driftBySeverity["critical"] > 0 {
		recommendations = append(recommendations, Recommendation{
			Title:       "Critical Drift Detected",
			Description: fmt.Sprintf("Found %d critical drift items that require immediate attention", driftBySeverity["critical"]),
			Priority:    "critical",
			Action:      "driftmgr remediation generate --severity critical",
			Impact:      "High risk to infrastructure stability and security",
			Category:    "immediate",
		})
	}
	...
	if hasSecurityDrift {
		recommendations = append(recommendations, Recommendation{
			...
			Description: "Security-related configuration has drifted from desired state",
			Priority:    "high",
			Action:      "Review and apply security group and encryption settings",
	...
	if hasNetworkDrift {
		recommendations = append(recommendations, Recommendation{
			Title:       "Network Configuration Drift",
			Description: "Network configuration has changed from terraform state",
			Priority:    "high",
			Action:      "Verify network connectivity and update terraform configuration",
			Impact:      "May affect application connectivity",
			Category:    "network",
		})
	}
	...
	if hasScalingDrift {
		recommendations = append(recommendations, Recommendation{
			Title:       "Resource Scaling Drift",
			Description: "Instance sizes or capacity settings have changed",
			Priority:    "medium",
			Action:      "Review auto-scaling policies and update terraform to match intended capacity",
			Impact:      "May affect performance and cost",
	...
	if hasTagDrift {
		recommendations = append(recommendations, Recommendation{
			Title:       "Resource Tagging Inconsistency",
			Description: "Resource tags have drifted from defined standards",
			Priority:    "low",
			Action:      "Update tags to match organizational tagging policy",
			Impact:      "May affect cost allocation and resource organization",
			Category:    "governance",
		})
	}

	// General recommendations based on drift percentage
	if report.Summary != nil {
		driftPercentage := float64(report.Summary.DriftedResources) / float64(report.Summary.TotalResources) * 100
		if driftPercentage > 30 {
			recommendations = append(recommendations, Recommendation{
				Title:       "High Drift Percentage",
				Description: fmt.Sprintf("%.1f%% of resources have drifted", driftPercentage),
				Priority:    "high",
				Action:      "Consider running terraform refresh and plan to resync state",
				Impact:      "Infrastructure state significantly diverged from code",
				Category:    "governance",
			})
		}
	}
	...
	if len(report.DriftItems) > 0 {
		recommendations = append(recommendations, Recommendation{
			Title:       "Enable Drift Detection Automation",
			Description: "Set up automated drift detection to catch changes early",
			Priority:    "medium",
			Action:      "Configure driftmgr server with scheduled scans",
			Impact:      "Proactive drift management",
			Category:    "automation",
		})

		recommendations = append(recommendations, Recommendation{
			Title:       "Implement Change Control",
			Description: "Ensure all infrastructure changes go through terraform",
			Priority:    "medium",
			Action:      "Review and restrict direct console/API access",
