...
type State struct {
	ID               string     `json:"id,omitempty"`        // Added for tracking
	Path             string     `json:"path,omitempty"`      // Added for file path
...
type Resource struct {
	...
	ID        string     `json:"id,omitempty"` // Resource ID for tracking
	Tags      interface{} `json:"tags,omitempty"` // Resource tags
}
...
type Instance struct {
	...
	DependsOn     []string               `json:"depends_on,omitempty"`  // Alias for Dependencies
}
...
// Loader provides state file loading capabilities
type Loader struct{}
