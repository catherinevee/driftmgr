...
func (e *Engine) Import(config Config) (*models.ImportResult, error) {
	...
	if config.GenerateConfig && result.Successful > 0 {
		...
		if err != nil {
			fmt.Printf("[WARNING]  Warning: Failed to generate Terraform configuration: %v\n", err)
		} else {
			fmt.Println("[OK] Generated Terraform configuration files")
		}
	}
	...
	if config.ValidateAfter && result.Successful > 0 {
		err := e.validateState()
		if err != nil {
			fmt.Printf("[WARNING]  Warning: State validation failed: %v\n", err)
		} else {
			fmt.Println("[OK] State validation passed")
		}
	}
...
func (e *Engine) loadResourcesFromCSV(filename string) ([]models.Resource, error) {
	...
	for _, record := range records[1:] {
		...
		for i, value := range record {
			...
			case "id":
				resource.ID = value
				importID = value // Default import ID to resource ID
			case "name":
				resource.Name = value
			case "type":
				resource.Type = value
		...
		// Store import fields in metadata
		if importID == "" {
		...
		resource.Metadata["import_id"] = importID
		resource.Metadata["terraform_type"] = terraformType

		// Validate required fields
		if resource.ID == "" || resource.Type == "" || importID == "" {
...
func (e *Engine) generateImportCommands(resources []models.Resource) []models.ImportCommand {
	...
	for _, resource := range resources {
		...
		terraformType := resource.Metadata["terraform_type"]
		if terraformType == "" {
			terraformType = resource.Type
		}
		importID := resource.Metadata["import_id"]
		if importID == "" {
			importID = resource.ID
		}
		...
		command := models.ImportCommand{
			...
			ResourceID:   importID,
			Command:      fmt.Sprintf("terraform import %s.%s %s", terraformType, resourceName, importID),
		}
...
func (e *Engine) generateTerraformConfig(commands []models.ImportCommand) error {
	...
	for resourceType, cmds := range configsByType {
		...
		for _, cmd := range cmds {
			...
			config := fmt.Sprintf("resource \"%s\" \"%s\" {\n", cmd.ResourceType, cmd.ResourceName)
			config += "  # Imported resource - update configuration as needed\n"
			config += "  lifecycle {\n"
			config += "    prevent_destroy = true\n"
			config += "  }\n"
			config += "}\n"
