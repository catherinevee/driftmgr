	...
	for pager.More() {
		...
		for _, account := range page.Value {
			if account != nil && account.ID != nil {
				// Determine actual state from SKU and properties
				state := "Unknown"
				if account.Properties != nil {
					if account.Properties.ProvisioningState != nil {
						state = string(*account.Properties.ProvisioningState)
					} else if account.Properties.StatusOfPrimary != nil {
						state = string(*account.Properties.StatusOfPrimary)
					}
				}
	...
	for pager.More() {
		...
		for _, group := range page.Value {
			if group != nil && group.ID != nil {
				// Get actual provisioning state from properties
				state := "Succeeded" // Default for resource groups
				if group.Properties != nil {
					// Type assert Properties to map to access ProvisioningState
					if props, ok := group.Properties.(map[string]interface{}); ok {
						if ps, exists := props["provisioningState"]; exists {
...
func (ap *AzureProvider) discoverApplicationInsights(ctx context.Context, accountID string) ([]models.Resource, error) {
	// Use Azure CLI to discover Application Insights components
	cmd := exec.Command("az", "monitor", "app-insights", "component", "list", "--subscription", accountID, "--output", "json")
	output, err := cmd.Output()
	if err != nil {
		// If Azure CLI is not available or command fails, return empty
		return []models.Resource{}, nil
	}

	var components []map[string]interface{}
	if err := json.Unmarshal(output, &components); err != nil {
		return []models.Resource{}, fmt.Errorf("failed to parse Application Insights components: %w", err)
	}
	...
	for _, component := range components {
		resource := models.Resource{
			...
			Location: getStringFromInterface(component["location"]),
			...
			Properties: map[string]interface{}{
				...
				"instrumentationKey":    getStringFromInterface(component["instrumentationKey"]),
				"connectionString":      getStringFromInterface(component["connectionString"]),
				"provisioningState":     getStringFromInterface(component["provisioningState"]),
				"requestSource":         getStringFromInterface(component["requestSource"]),
				"retentionInDays":       getIntFromInterface(component["retentionInDays"]),
				"samplingPercentage":    getFloatFromInterface(component["samplingPercentage"]),
				"workspaceResourceId":   getStringFromInterface(component["workspaceResourceId"]),
				"publicNetworkAccess":   getStringFromInterface(component["publicNetworkAccessForIngestion"]),
				"disableIpMasking":      getBoolFromInterface(component["disableIpMasking"]),
				"immediatePurgeEnabled": getBoolFromInterface(component["immediatePurgeDataOn30Days"]),
			},
		}
...
func (ap *AzureProvider) deleteResourceGroup(ctx context.Context, resource models.Resource) error {
	...
	client, err := armresources.NewResourceGroupsClient(ap.subscriptionID, ap.cred, nil)
	if err != nil {
	...
	for pager.More() {
		page, err := pager.NextPage(ctx)
		if err != nil {
			return fmt.Errorf("failed to list resources in resource group: %w", err)
		}
...
func extractTagsFromInterface(v interface{}) map[string]string {
	...
	case map[string]interface{}:
		for k, v := range t {
			tags[k] = getStringFromInterface(v)
		}
