...
type RemediationExecutor struct {
	...
	backupManager  *state.BackupManager
...
type ExecutionHooks struct {
	BeforeAction func(action *RemediationAction) error
	AfterAction  func(action *RemediationAction, result ActionResult) error
	OnError      func(action *RemediationAction, err error) error
	OnRollback   func(plan *RemediationPlan) error
}
...
func NewRemediationExecutor(stateManager *state.StateManager, workDir string) *RemediationExecutor {
	return &RemediationExecutor{
		stateManager:   stateManager,
		backupManager:  state.NewBackupManager(filepath.Join(workDir, "backups")),
...
func (re *RemediationExecutor) Execute(ctx context.Context, plan *RemediationPlan) (*ExecutionResult, error) {
	if plan == nil {
...
func (re *RemediationExecutor) executeAction(ctx context.Context, action *RemediationAction) ActionResult {
	result := ActionResult{
		ActionID:   action.ID,
		ResourceID: action.Resource,
		Action:     string(action.Type),
	...
	if re.dryRun {
		result.Status = StatusSuccess
		result.EndTime = time.Now()
		result.Output = fmt.Sprintf("[DRY RUN] Would execute: %s on %s", action.Type, action.Resource)
		return result
	}
...
func (re *RemediationExecutor) executeCreateAction(ctx context.Context, action *RemediationAction, result *ActionResult) error {
	...
	configFile := filepath.Join(re.workDir, fmt.Sprintf("create_%s.tf", action.Resource))
	...
	// Run terraform apply
	cmd := exec.CommandContext(ctx, re.terraformPath, "apply", "-auto-approve", "-target", action.Resource)
	cmd.Dir = re.workDir
	...
	result.Changes = append(result.Changes, fmt.Sprintf("Created resource %s", action.Resource))
	return nil
}

func (re *RemediationExecutor) executeUpdateAction(ctx context.Context, action *RemediationAction, result *ActionResult) error {
	...
	result.Changes = append(result.Changes, fmt.Sprintf("Updated resource %s", action.Resource))
	return nil
}

func (re *RemediationExecutor) executeDeleteAction(ctx context.Context, action *RemediationAction, result *ActionResult) error {
	// Run terraform destroy for the specific resource
	cmd := exec.CommandContext(ctx, re.terraformPath, "destroy", "-auto-approve", "-target", action.Resource)
	cmd.Dir = re.workDir
	output, err := cmd.CombinedOutput()
	result.Output = string(output)
	...
	result.Changes = append(result.Changes, fmt.Sprintf("Deleted resource %s", action.Resource))
	return nil
}

func (re *RemediationExecutor) executeImportAction(ctx context.Context, action *RemediationAction, result *ActionResult) error {
	...
	cmd := exec.CommandContext(ctx, re.terraformPath, "import", action.Resource, importID)
	cmd.Dir = re.workDir
	output, err := cmd.CombinedOutput()
	result.Output = string(output)
	...
	result.Changes = append(result.Changes, fmt.Sprintf("Imported resource %s with ID %s", action.Resource, importID))
	return nil
}

func (re *RemediationExecutor) executeRefreshAction(ctx context.Context, action *RemediationAction, result *ActionResult) error {
	// Run terraform refresh
	cmd := exec.CommandContext(ctx, re.terraformPath, "refresh", "-target", action.Resource)
	cmd.Dir = re.workDir
	output, err := cmd.CombinedOutput()
	result.Output = string(output)
	...
	result.Changes = append(result.Changes, fmt.Sprintf("Refreshed resource %s", action.Resource))
	return nil
}

func (re *RemediationExecutor) executeTaintAction(ctx context.Context, action *RemediationAction, result *ActionResult) error {
	// Run terraform taint
	cmd := exec.CommandContext(ctx, re.terraformPath, "taint", action.Resource)
	cmd.Dir = re.workDir
	output, err := cmd.CombinedOutput()
	result.Output = string(output)
	...
	result.Changes = append(result.Changes, fmt.Sprintf("Tainted resource %s", action.Resource))
	return nil
}

func (re *RemediationExecutor) executeUntaintAction(ctx context.Context, action *RemediationAction, result *ActionResult) error {
	// Run terraform untaint
	cmd := exec.CommandContext(ctx, re.terraformPath, "untaint", action.Resource)
	cmd.Dir = re.workDir
	output, err := cmd.CombinedOutput()
	result.Output = string(output)
	...
	result.Changes = append(result.Changes, fmt.Sprintf("Untainted resource %s", action.Resource))
	return nil
}

func (re *RemediationExecutor) executeMoveAction(ctx context.Context, action *RemediationAction, result *ActionResult) error {
	...
	cmd := exec.CommandContext(ctx, re.terraformPath, "state", "mv", action.Resource, newID)
	cmd.Dir = re.workDir
	output, err := cmd.CombinedOutput()
	result.Output = string(output)
	...
	result.Changes = append(result.Changes, fmt.Sprintf("Moved resource from %s to %s", action.Resource, newID))
	return nil
}

func (re *RemediationExecutor) canExecuteAction(action *RemediationAction, completedActions []ActionResult) bool {
	// Check dependencies
...
func (re *RemediationExecutor) createBackup(ctx context.Context, backupID string) error {
	// Get current state
	state, err := re.stateManager.GetState(ctx, "default")
	if err != nil {
...
func (re *RemediationExecutor) executeRollback(ctx context.Context, plan *RemediationPlan, executedActions []*RemediationAction, backupID string) error {
	...
	// Execute rollback if there's a rollback plan
	if plan.RollbackPlan != nil {
		// Execute rollback actions in reverse order
		for i := len(plan.RollbackPlan.Actions) - 1; i >= 0; i-- {
			rollbackAction := plan.RollbackPlan.Actions[i]
			// Create a RemediationAction from RollbackAction for execution
			remAction := &RemediationAction{
				ID:          rollbackAction.ID,
				Type:        ActionTypeUpdate, // Using update as fallback type
				Description: rollbackAction.Description,
				Command:     rollbackAction.Command,
				Parameters:  rollbackAction.Parameters,
			}
			re.executeAction(ctx, remAction)
		}
	}
