...
func NewEnhancedDiscoverer(cfg *config.Config) *EnhancedDiscoverer {
	...
	return &EnhancedDiscoverer{
		config:          cfg,
		// cache:           nil, // Cache not implemented yet
...
func (ed *EnhancedDiscoverer) discoverGCPCloudDebugger(ctx context.Context, region string) []models.Resource {
	...
	if err := json.Unmarshal(output, &targets); err != nil {
		log.Printf("Error parsing Cloud Debugger output: %v", err)
		return resources
	}

	// Convert to resource models
	for _, target := range targets {
		if targetID, ok := target["targetId"].(string); ok {
			resource := models.Resource{
				ID:           fmt.Sprintf("debugger-%s", targetID),
				Name:         targetID,
