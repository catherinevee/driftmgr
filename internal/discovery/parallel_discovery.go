...
func (pd *ParallelDiscoverer) performDiscovery(ctx context.Context, provider, region string) ([]models.Resource, error) {
	...
	case "azure":
		// For Azure, we need subscription ID and resource group
		// These should come from config or environment
		azureProvider := providers.NewAzureProviderComplete("", "", "", "", "", region)
		resources, err := azureProvider.DiscoverResources(ctx, region)
		if err != nil {
			return nil, fmt.Errorf("failed to discover Azure resources: %w", err)
		}
		return resources, nil
	case "gcp":
		// For GCP, we need project ID
		// This should come from config or environment
		gcpProvider := providers.NewGCPProviderComplete("", region, "")
		resources, err := gcpProvider.DiscoverResources(ctx, region)
		if err != nil {
...
func (pd *ParallelDiscoverer) Shutdown(timeout time.Duration) error {
	// Since we're using simple channels, just return nil
