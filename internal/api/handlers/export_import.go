...
func (s *Server) exportResourcesJSON(w http.ResponseWriter, resources []models.Resource) {
...
func (s *Server) exportResourcesCSV(w http.ResponseWriter, resources []models.Resource) {
	...
	for _, resource := range resources {
		...
		if resource.Tags != nil {
			if tagsMap, ok := resource.Tags.(map[string]string); ok {
...
func (s *Server) exportResourcesTerraform(w http.ResponseWriter, resources []models.Resource) {
	...
	for provider, providerResources := range byProvider {
		...
		for _, resource := range providerResources {
			...
			if resource.Tags != nil {
				if tagsMap, ok := resource.Tags.(map[string]string); ok && len(tagsMap) > 0 {
					fmt.Fprintf(w, "    tags = {\n")
...
func (s *Server) validateResource(resource models.Resource) error {
	if resource.ID == "" {
