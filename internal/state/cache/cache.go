...
func (c *StateCache) Get(key string) *parser.TerraformState {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *StateCache) Set(key string, state *parser.TerraformState) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *StateCache) calculateSize(state *parser.TerraformState) int64 {
