...
import (
	...
	"github.com/catherinevee/driftmgr/internal/state/cache"
	"github.com/catherinevee/driftmgr/internal/state/parser"
	"github.com/catherinevee/driftmgr/internal/state/validator"
)
...
func NewStateManager(backend backend.Backend) *StateManager {
	return &StateManager{
		backend:   backend,
		parser:    parser.NewParser(),
		validator: validator.NewValidator(),
		cache:     cache.NewStateCache(1 * time.Hour),
	}
}

// GetState retrieves and parses a state file
func (sm *StateManager) GetState(ctx context.Context, key string) (*parser.TerraformState, error) {
	// Check cache first
	...
	// Get from backend
	data, err := sm.backend.GetState(ctx, key)
	if err != nil {
...
func (sm *StateManager) UpdateState(ctx context.Context, key string, state *parser.TerraformState) error {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	...
	// Lock state
	lockID, err := sm.backend.LockState(ctx, key)
	if err != nil {
		return fmt.Errorf("failed to lock state: %w", err)
	}
	defer sm.backend.UnlockState(ctx, key, lockID)
	...
	// Upload to backend
	if err := sm.backend.PutState(ctx, key, data); err != nil {
...
func (sm *StateManager) DeleteState(ctx context.Context, key string) error {
	...
	// Delete from backend
	if err := sm.backend.DeleteState(ctx, key); err != nil {
...
func (sm *StateManager) GetStateHistory(ctx context.Context, key string) ([]registry.StateVersion, error) {
...
func (sm *StateManager) RestoreStateVersion(ctx context.Context, key string, version int) error {
	...
	if err := sm.backend.PutState(ctx, key, newData); err != nil {
...
func (sm *StateManager) ImportResource(ctx context.Context, key string, resource parser.Resource) error {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	...
	if err != nil {
		...
		state = &parser.TerraformState{
			...
			Outputs:   make(map[string]parser.OutputValue),
		}
	}
...
func (sm *StateManager) RemoveResource(ctx context.Context, key string, resourceAddress string) error {
	...
	found := false
	newResources := make([]parser.Resource, 0, len(state.Resources)-1)
...
func (sm *StateManager) MoveResource(ctx context.Context, key string, fromAddress, toAddress string) error {
	...
	var sourceResource *parser.Resource
	newResources := make([]parser.Resource, 0, len(state.Resources))
...
func (sm *StateManager) compareStateObjects(state1, state2 *parser.TerraformState) []StateDifference {
