...
// Format formats the report as PDF using gofpdf
func (f *PDFFormatter) Format(report *ComplianceReport) ([]byte, error) {
	...
	pdf.Cell(0, 6, fmt.Sprintf("Report ID: %s", report.ID))
	pdf.Ln(6)
	pdf.Cell(0, 6, fmt.Sprintf("Generated: %s", report.GeneratedAt.Format("2006-01-02 15:04:05")))
	pdf.Ln(6)
	...
	pdf.SetFont("Arial", "", 10)
	pdf.Cell(0, 6, fmt.Sprintf("Total Controls: %d", report.Summary.TotalControls))
	pdf.Ln(6)
	pdf.Cell(0, 6, fmt.Sprintf("Passed Controls: %d", report.Summary.PassedControls))
	pdf.Ln(6)
	pdf.Cell(0, 6, fmt.Sprintf("Failed Controls: %d", report.Summary.FailedControls))
	pdf.Ln(6)
	pdf.Cell(0, 6, fmt.Sprintf("Compliance Score: %.1f%%", report.Summary.ComplianceScore))
	pdf.Ln(12)
	...
	pdf.SetFont("Arial", "", 10)
	pdf.Cell(0, 6, fmt.Sprintf("Critical Findings: %d", report.Summary.CriticalFindings))
	pdf.Ln(6)
	pdf.Cell(0, 6, fmt.Sprintf("High Findings: %d", report.Summary.HighFindings))
	pdf.Ln(6)
	pdf.Cell(0, 6, fmt.Sprintf("Medium Findings: %d", report.Summary.MediumFindings))
	pdf.Ln(6)
	pdf.Cell(0, 6, fmt.Sprintf("Low Findings: %d", report.Summary.LowFindings))
	pdf.Ln(12)
	...
	for _, section := range report.Sections {
		...
		if section.Description != "" {
			pdf.SetFont("Arial", "", 10)
			pdf.Cell(0, 6, section.Description)
			pdf.Ln(6)
		}
		...
		for _, control := range section.Controls {
			...
			pdf.Cell(0, 6, fmt.Sprintf("%s - %s", control.ID, control.Title))
			pdf.Ln(6)

			pdf.SetFont("Arial", "", 9)
			pdf.Cell(0, 5, control.Description)
			pdf.Ln(5)
			...
			pdf.Cell(0, 5, fmt.Sprintf("Status: %s", string(control.Status)))
			pdf.Ln(5)
			...
			if len(control.Findings) > 0 {
				...
				for _, finding := range control.Findings {
					pdf.SetFont("Arial", "", 8)
					pdf.Cell(0, 4, fmt.Sprintf("• %s (%s): %s", finding.Title, finding.Severity, finding.Description))
					pdf.Ln(4)

					if finding.Remediation != "" {
						pdf.Cell(0, 4, fmt.Sprintf("  Remediation: %s", finding.Remediation))
						pdf.Ln(4)
					}
				}
			}
	...
	// Footer
	pdf.SetFont("Arial", "", 8)
	pdf.Cell(0, 6, "This report was automatically generated by DriftMgr Compliance Reporter")
	pdf.Ln(6)

	if report.Signature != "" {
		pdf.Cell(0, 6, fmt.Sprintf("Digital Signature: %s", report.Signature))
	}
