...
func (udb *UserDB) GetUserByID(userID string) (*User, error) {
	query := `
		SELECT id, username, password, role, created, last_login, failed_login_attempts, 
	...
	err := udb.db.QueryRow(query, userID).Scan(
		&user.ID, &user.Username, &user.Password, &user.Role,
		&user.Created, &user.LastLogin, &user.FailedLoginAttempts,
		&lockedUntil, &user.MFAEnabled, &user.MFASecret,
	)
