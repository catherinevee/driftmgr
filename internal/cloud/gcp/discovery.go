...
// Config represents GCP discovery configuration
type Config struct {
	Regions      []string
	ResourceType string
}
...
func (p *GCPProvider) Discover(config Config) ([]models.Resource, error) {
	...
	if p.projectID == "" {
		...
		if p.projectID == "" {
			return nil, fmt.Errorf("GCP project ID not configured. Please set GOOGLE_CLOUD_PROJECT environment variable or configure gcloud")
		}
	}
